...
def get_holiday_cache():
    # Robustno učitavanje CroatianHolidays.py i prikaz detaljne greške
    candidates = []
    here = Path(__file__).resolve().parent
    candidates.append(here / "CroatianHolidays.py")
    candidates.append(Path.cwd() / "CroatianHolidays.py")
    # dodaj i roditeljski dir ako se app pokreće iz podmape
    candidates.append(here.parent / "CroatianHolidays.py")
    # ukloni duplikate i nepostojeće
    uniq = []
    seen = set()
    for p in candidates:
        try:
    ...
    for path in uniq:
        try:
            spec = importlib.util.spec_from_file_location("CroatianHolidays", str(path))
            mod = importlib.util.module_from_spec(spec)
            sys.modules["CroatianHolidays"] = mod
            spec.loader.exec_module(mod)  # može dići grešku iz modula
            if hasattr(mod, "CroatianHolidays"):
                ch = getattr(mod, "CroatianHolidays")()
                return ch, {}
            else:
                raise AttributeError("Modul ne sadrži klasu 'CroatianHolidays'.")
        except Exception as e:
            last_exc = (path, e)
            continue
    # Ako nismo uspjeli
    if last_exc:
        path, e = last_exc
        st.error(f"Nisam uspio učitati CroatianHolidays.py s lokacije: {path}")
        st.exception(e)
    else:
        st.error("CroatianHolidays.py nije pronađen uz app.py. Datoteka mora biti u istom folderu.")
    st.stop()
...
def holiday_name_for_date(d: date, ch, cache):
    ...
    if y not in cache:
        try:
            holidays = ch.getHolidays(y)
        except Exception as e:
            st.warning(f"Ne mogu dobiti blagdane za {y}. ({e})")
            holidays = {}
        cache[y] = holidays if isinstance(holidays, dict) else {}
