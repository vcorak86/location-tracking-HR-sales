# app.py (v7.2) — Test Connection + prošli zapisi + svi CSV u /data
...
def load_employees(path:str):
    ...
    if not need.issubset(df.columns): st.error(f"U '{path}' moraju postojati kolone: {', '.join(sorted(need))}."); st.stop()
...
def load_locations(path:str):
    ...
    vals=[str(x).strip().strip(' )').strip() for x in df[df.columns[0]].tolist()]
    out=[]; 
    for v in vals:
        if not v: continue
...
def normalize_columns(df:pd.DataFrame)->pd.DataFrame:
    ren={c:re.sub(r"\s+"," ",c.strip()).lower() for c in df.columns}
    t=df.rename(columns=ren); m={}
    for c in t.columns:
    ...
    return t.rename(columns=m)

def dedupe_on_name_date(df:pd.DataFrame)->pd.DataFrame:
    t=df.copy()
    if "Datum" in t.columns: t["Datum_norm"]=pd.to_datetime(t["Datum"], dayfirst=True, errors="coerce").dt.strftime("%d.%m.%Y.")
    else: t["Datum_norm"]=None
    if "Ime i prezime" in t.columns: t=t.drop_duplicates(subset=["Ime i prezime","Datum_norm"], keep="last")
    return t
...
def load_tracker()->pd.DataFrame:
    if gh_enabled():
        ...
        if content is None:
            ...
            if seed_path.exists():
                put_res=gh_put_file(cfg["repo"], cfg["path"], cfg["branch"], seed_path.read_bytes(), "Seed data/Tracker.csv from app",
        ...
        gh_df,_=parse_csv_bytes(content, preferred_sep=_gh_config()["csv_sep"]); gh_df=normalize_columns(gh_df)
        try: LOCAL_FALLBACK_LOG.parent.mkdir(parents=True, exist_ok=True); gh_df.to_csv(LOCAL_FALLBACK_LOG, index=False)
        except Exception: pass
        return gh_df
    else:
        if LOCAL_FALLBACK_LOG.exists(): df=pd.read_csv(LOCAL_FALLBACK_LOG); return normalize_columns(df)
        return pd.DataFrame(columns=["Datum","Ime i prezime","Odjel","Lokacija","Week","Month","Year"])

def save_tracker(new_rows:pd.DataFrame):
    existing=load_tracker(); nr=normalize_columns(new_rows)
    merged=dedupe_on_name_date(pd.concat([existing,nr], ignore_index=True))
    ...
    if gh_enabled():
        ...
        put_res=gh_put_file(cfg["repo"], cfg["path"], cfg["branch"], out, "Update data/Tracker.csv from Streamlit", sha,
                            cfg["committer_name"], cfg["committer_email"])
        if put_res.get("status") not in (200,201): st.error(f"GitHub PUT error {put_res.get('status')}"); st.caption(put_res.get("body"))
    else: st.info("GitHub sync nije konfiguriran; ažuriran je samo lokalni keš.")
...
with st.expander("🔌 Test connection (za administratore)"):
    code=st.text_input("Unesite šifru", type="password", value="")
    if code=="12345":
        st.success("Šifra ispravna.")
        if gh_enabled():
            cfg=_gh_config()
            st.write(f"Repo: `{cfg['repo']}`  |  Branch: `{cfg['branch']}`  |  Path: `{cfg['path']}`  |  CSV sep: `{cfg['csv_sep']}`")
            status_repo, body_repo = gh_repo_info(cfg["repo"]); st.write(f"Repo GET status: {status_repo}")
            if status_repo!=200: st.code(body_repo)
            if st.button("▶️ Test connection"):
    ...
    elif code: st.error("Pogrešna šifra.")
...
# Prikaži prošle zapise za korisnika
tracker_all=load_tracker()
if not tracker_all.empty:
    t=normalize_columns(tracker_all).copy()
    try: t["Datum_d"]=pd.to_datetime(t["Datum"], dayfirst=True, errors="coerce")
    except Exception: t["Datum_d"]=pd.NaT
    mine=t[t["Ime i prezime"]==full_name].copy()
    if not mine.empty:
        mine=mine.sort_values("Datum_d", ascending=False)
        ...
        st.dataframe(mine[show], use_container_width=True, hide_index=True)
    else: st.info("Nema prijašnjih zapisa u Tracker.csv za ovog korisnika.")
...
def get_week_monday(ref: date, offset_weeks:int)->date: return (pd.Timestamp(ref)-pd.Timedelta(days=ref.weekday())+pd.Timedelta(weeks=offset_weeks)).date()
...
prefill={}
if not tracker_all.empty:
    t=normalize_columns(tracker_all)
...
if not tracker_df.empty:
    ...
    try: t["Datum"]=pd.to_datetime(t["Datum"], dayfirst=True, errors="coerce")
    except Exception: pass
    mine=t[(t["Ime i prezime"]==full_name) & (t["Year"]==date.today().year)]
    if not mine.empty:
        counts=mine["Lokacija"].value_counts()
        fig,ax=plt.subplots(); ax.pie(counts.values, labels=counts.index, autopct="%1.1f%%", startangle=90); ax.axis("equal"); st.pyplot(fig)
