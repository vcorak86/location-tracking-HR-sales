...
# --- helpers ---
def _safe_str(x) -> str:
    """Pretvori u string; NaN/None -> '' (prazno)."""
    if x is None:
        return ""
    try:
        if pd.isna(x):
...
def normalize_columns(df: pd.DataFrame) -> pd.DataFrame:
    ...
    for c in t.columns:
        if "datum" in c:
            m[c] = "Datum"
        elif ("ime" in c and "prezime" in c) or c == "ime i prezime" or c == "name":
            # mapiraj i 'name' u 'Ime i prezime' radi starijih dataset-a
            m[c] = "Ime i prezime"
        elif "odjel" in c:
            m[c] = "Odjel"
        elif "lokacija" in c:
            ...
            m[c] = "Dan"
        elif c in ("record_id", "created_at", "updated_at", "source", "version", "date_iso"):
            m[c] = c
        else:
            m[c] = c
...
# ---------- Record id / versioning ----------
def record_key(name: str, date_iso: str) -> str:
    name_s = _safe_str(name).strip()
    date_s = _safe_str(date_iso)
    return f"{name_s}|{date_s}"
...
def validate_tracker_schema(df: pd.DataFrame) -> list[str]:
    ...
    required = [
        "Datum", "Ime i prezime", "Odjel", "Lokacija",
        "Week", "Month", "Year",
        "date_iso", "record_id", "created_at", "updated_at", "source", "version",
    ]
    missing = [c for c in required if c not in t.columns]
    problems = []
    if missing:
        problems.append(f"Nedostaju kolone: {missing}")
    # unique per (Ime i prezime, date_iso)
    dup = t.duplicated(subset=["Ime i prezime", "date_iso"], keep=False) if all(c in t.columns for c in ["Ime i prezime", "date_iso"]) else pd.Series(False, index=t.index)
    if dup.any():
    ...
    if "date_iso" in t.columns:
        ...
        if bad_date.any():
            problems.append("Nevaljani date_iso zapisi postoje.")
    return problems
